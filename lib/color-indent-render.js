...
const getColor = ({ indentation, numberOfColors, chosenColor }) =>
  pSBC((indentation * (-100 / numberOfColors)) / 100, chosenColor);
...
const paintTextEditorLine = (textEditor, {
  ...
  const indentation = getIndentationColor(textEditor, { line, numberOfColors });
  ...
  if (marker.isValid() && !marker.isDestroyed()) {
    const item = el('div', {
      style: {
        backgroundColor: getColor({ indentation, numberOfColors, chosenColor }),
        width: chosenWidth,
      },
    });
