...
const getIndentationColor = (textEditor, { line, chosenIndentationDepth }) => {
  ...
  if (indentation > (chosenIndentationDepth - 1)) {
    return chosenIndentationDepth - 1;
  }
...
const getColor = ({ indentation, chosenIndentationDepth, chosenColor }) =>
  pSBC((indentation * (-100 / chosenIndentationDepth)) / 100, chosenColor);
...
const paintTextEditorLine = (textEditor, {
  ...
  if (marker.isValid() && !marker.isDestroyed()) {
    const item = el('div', {
      style: {
        backgroundColor: getColor({ indentation, chosenIndentationDepth, chosenColor }),
        width: chosenWidth,
      },
    });
